...
function renderDashboardMetrics() {
  ...
  const offline = state.equipment.filter((item) => item.status === "offline").length;
  ...
  const metrics = [
    {label: "設備總數", value: state.equipment.length, hint: `可用 ${available} 台`},
    {label: "本週預約", value: activeReservations.length, hint: `${reservedHours.toFixed(1)} 小時已排程`},
    {label: "維修中", value: maintenance, hint: "不可建立新預約"},
    {label: "停用", value: offline, hint: "暫停服務"},
  ];
  ...
  state.equipment.forEach((item) => {
    ...
    card.innerHTML = `
      <div>
        ...
        <p>${escapeHtml(item.category)} · ${escapeHtml(item.location || "未設定位置")} · 容量 ${item.capacity}</p>
      </div>
      <label class="equipment-status-control">
        <span>狀態</span>
        <select class="equipment-status-select" data-equipment-id="${item.id}">
          <option value="available" ${item.status === "available" ? "selected" : ""}>可預約</option>
          <option value="maintenance" ${item.status === "maintenance" ? "selected" : ""}>維修中</option>
          <option value="offline" ${item.status === "offline" ? "selected" : ""}>停用</option>
        </select>
      </label>
    `;
    card.querySelector(".equipment-status-select").addEventListener("change", updateEquipmentStatus);
    root.appendChild(card);
  });
}

async function updateEquipmentStatus(event) {
  const select = event.currentTarget;
  const equipmentId = select.dataset.equipmentId;
  const status = select.value;
  select.disabled = true;
  try {
    await api(`/api/equipment/${equipmentId}`, {
      method: "PATCH",
      body: JSON.stringify({status, changed_by: "Lab Admin"}),
    });
    await loadEquipment();
    renderAll();
  } catch (error) {
...
function renderGantt() {
  ...
  state.equipment.forEach((equipment) => {
    ...
    label.innerHTML = `
      ...
      <span>${escapeHtml(equipment.category)} · ${statusText[equipment.status] || equipment.status}</span>
    `;
...
function renderReservationRows() {
  ...
  state.reservations.forEach((reservation) => {
    ...
    tr.innerHTML = `
      ...
      <td><span class="badge ${escapeHtml(reservation.status)}">${statusText[reservation.status] || reservation.status}</span></td>
